...
def evaluate_from_dict(gt_entry, pred_entry, mode, result_dict, multiple_preds=False,
    ...
    rel_scores = pred_entry['rel_scores']
     ...
     # 🔑 处理空关系的情况
    original_gt_rels_count = gt_rels.shape[0]
    if original_gt_rels_count == 0:
        print("⚠️ gt_rels 为空，创建默认关系用于评估")
        # 创建一个默认关系避免除零错误
        if gt_classes.shape[0] >= 2:
            # 如果有至少2个对象，创建第一个到第二个的关系
            gt_rels = np.array([[0, 1, 1]], dtype=np.int32)
            print(f"  创建默认关系: {gt_rels}")
        elif gt_classes.shape[0] >= 1:
            # 如果只有一个对象，创建自环关系
            gt_rels = np.array([[0, 0, 1]], dtype=np.int32)
            print(f"  创建自环关系: {gt_rels}")
        else:
            # 如果没有对象，跳过评估
            print("  没有对象，跳过评估")
            # 为结果字典添加零值
...
def evaluate_recall(gt_rels, gt_boxes, gt_classes,
    ...
    if num_gt_relations == 0:
        gt_rels = np.ones((1, 3), dtype=np.int32)
        num_gt_relations = 1
    # print("gt_rels: ", gt_rels)
    # print("Number of GT relations: ", num_gt_relations)
