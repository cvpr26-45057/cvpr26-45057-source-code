...
def eval_rel_results(all_results, topk=100, do_val=True, do_vis=False, rel_categories=None):
    ...
    if do_val:
        ...
        if eval_ap:
            ...
            if rel_categories is not None:
                # 使用传入的关系类别
                rel_prd_cats = rel_categories
                num_rel_classes = len(rel_prd_cats)
                print(f"📊 使用传入的关系类别: {rel_prd_cats} (共{num_rel_classes}个)")
            else:
                # 尝试从VG数据集文件加载（向后兼容）
                try:
                    with open('data/vg/rel.json') as f: #TODO relative path!!!!!!!!!!!
                        rel_data = json.load(f)
                        rel_prd_cats = rel_data['rel_categories']
                        num_rel_classes = len(rel_prd_cats)
                except FileNotFoundError:
                    print("⚠️ 未找到VG关系文件，使用默认关系类别")
                    rel_prd_cats = ['default_relation']
                    num_rel_classes = len(rel_prd_cats)
            
            cls_image_ids, cls_dets, cls_gts, npos = prepare_mAP_dets(topk_dets, num_rel_classes)
            all_npos = sum(npos)

            rel_mAP = 0.
            w_rel_mAP = 0.
            ap_str = ''
            for c in range(num_rel_classes):
                ...
                print('rel AP for class {}: {:.2f} ({:.6f})'.format(rel_prd_cats[c] if c < len(rel_prd_cats) else f'class_{c}', 100 * ap, float(npos[c]) / float(all_npos)))
            rel_mAP /= num_rel_classes
            ...
            for c in range(num_rel_classes):
                ...
                phr_mAP += ap
                ap_str += '{:.2f}, '.format(100 * ap)
                print('phr AP for class {}: {:.2f} ({:.6f})'.format(rel_prd_cats[c] if c < len(rel_prd_cats) else f'class_{c}', 100 * ap, float(npos[c]) / float(all_npos)))
            phr_mAP /= num_rel_classes
