...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, entity_embed, triplet_embed, pos_embed, so_embed, tgt_mask=None, triplet_entity_key_padding_mask=None):
        # flatten NxCxHxW to HWxNxC
        bs, c, h, w = src.shape
        src = src.flatten(2).permute(2, 0, 1)
        pos_embed = pos_embed.flatten(2).permute(2, 0, 1)

        if entity_embed.dim() == 2:
            entity_embed = entity_embed.unsqueeze(0).expand(bs, -1, -1)
        elif entity_embed.dim() == 3:
            if entity_embed.size(0) != bs:
                if entity_embed.size(0) == 1:
                    entity_embed = entity_embed.expand(bs, -1, -1)
                else:
                    raise ValueError("entity_embed batch dimension mismatch with src")
        else:
            raise ValueError("entity_embed must have 2 or 3 dimensions")

        if triplet_embed.dim() == 2:
            triplet_embed = triplet_embed.unsqueeze(0).expand(bs, -1, -1)
        elif triplet_embed.dim() == 3 and triplet_embed.size(0) == 1:
            triplet_embed = triplet_embed.expand(bs, -1, -1)
        elif triplet_embed.dim() != 3 or triplet_embed.size(0) != bs:
            raise ValueError("triplet_embed must align with batch size")

        entity_pos, entity = torch.split(entity_embed, c, dim=-1)
        triplet_pos, triplet = torch.split(triplet_embed, [c, 2 * c], dim=-1)

        entity_pos = entity_pos.permute(1, 0, 2)
        entity = entity.permute(1, 0, 2)
        triplet_pos = triplet_pos.permute(1, 0, 2)
        triplet = triplet.permute(1, 0, 2)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, return_intermediate=False, use_deformable_attn: bool = False, use_grad_checkpoint: bool = False):
        super().__init__()
    ...
    def forward(self, entity, triplet, memory,
                ...
                triplet_entity_key_padding_mask: Optional[Tensor] = None):
        ...
        for layer in self.layers:
            if self.use_grad_checkpoint and output_entity.requires_grad and output_triplet.requires_grad:
                def layer_forward(ent, trip):
                    return layer(
                        ent,
                        trip,
                        entity_pos,
                ...
                output_entity, output_triplet, sub_maps, obj_maps = checkpoint(
                    layer_forward,
                    output_entity,
                    output_triplet,
                    use_reentrant=False,
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", use_deformable_attn: bool = False):
        super().__init__()
        self.activation = _get_activation_fn(activation)
        self.use_deformable_attn = use_deformable_attn
        ...
        if use_deformable_attn:
            ...
            self.cross_attn_entity = MultiScaleDeformableAttention(d_model, nhead, n_levels=1, n_points=4)
            self.entity_reference_points = nn.Linear(d_model, 2)
        else:
    ...
    def forward(self, tgt_entity, tgt_triplet, entity_pos, triplet_pos, so_pos,
        ...
        if self.use_deformable_attn:
            B = tgt_entity.size(1)
            query = self.with_pos_embed(tgt_entity, entity_pos).transpose(0, 1)  # [B,Q,C]
            memory_flatten = memory.permute(1, 0, 2)  # [B, HW, C]
            spatial_shapes = torch.as_tensor([[memory_hw[0], memory_hw[1]]], dtype=torch.long, device=memory.device)
            level_start_index = torch.as_tensor([0], dtype=torch.long, device=memory.device)
            reference = torch.sigmoid(self.entity_reference_points(query)).unsqueeze(2)  # [B,Q,1,2]
        ...
        tgt2_sub = self.cross_sub_entity(query=self.with_pos_embed(tgt_sub, triplet_pos),
                                         ...
                                         key_padding_mask=triplet_entity_key_padding_mask)[0]
...
def build_transformer(args):
    return Transformer(
        ...
        use_deformable_attn=getattr(args, "use_deformable_attn", False),
